"""Plant pipeline for artist discographies on Lyrics WSD v18.

Enhancements in v18:
1. Full-Corpus Line Back-Search: indexes all authentic lyric lines across the artist discography
   to rescue words whose 'm' and 'r' arrays were unpopulated in v7, achieving 0 synthetic dummy lines.
2. Sanitized Wikipedia Entity Resolver: strict blocklists and contextual checks eliminate
   false positive airport, metal band, and ancient monarch disambiguations.
3. Noise Sense Pruning: drops alphabet letter names, secondary school acronyms (ESO),
   and spurious telephone greetings.
4. Human Clitic Gloss Translation: translates inflected enclitic verbs to idiomatic English.
5. Ingests all expanded GRAFT overlays across Caribbean slang, Peninsular/flamenco idioms,
   and conversational discourse particles.
6. Code-switching and language origin tags (is_english, is_catalan, is_code_switching).
7. Publishes both individual artist releases and the unified 'lyrics-all-artists-v18' release.
"""
...
def build_corpus_line_index(raw_examples: dict[str, dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
    """Index all authentic lyric lines across all cards by lowercase token."""
    print("Indexing entire artist lyrics corpus for line back-search...")
    index: dict[str, list[dict[str, Any]]] = defaultdict(list)
    seen_spanish: set[str] = set()

    for d in raw_examples.values():
        for b in d.get("m", []):
            for item in b:
                sp = item.get("spanish", "").strip()
                if sp and sp not in seen_spanish and item.get("song_name") not in {"Unknown", "Lyrical Context", None, ""}:
                    seen_spanish.add(sp)
                    tokens = set(re.findall(r"\b[\w'’áéíóúüñ]+\b", sp.lower()))
                    for tok in tokens:
                        index[tok].append(item)
        for item in d.get("r", []):
            sp = item.get("spanish", "").strip()
            if sp and sp not in seen_spanish and item.get("song_name") not in {"Unknown", "Lyrical Context", None, ""}:
                seen_spanish.add(sp)
    ...
    print(f"  Indexed {len(seen_spanish):,} distinct authentic lyric lines across {len(index):,} vocabulary tokens.")
    return index
...
def plant_artist(
    ...
    print("\n================================================================================")
    print(f"Lyrics WSD v18 Plant Pipeline: {artist_slug}")
    ...
    # Build corpus line index for 0-dummy line guarantee
    ...
    # 5. Occurrence Sampling Sieve with Full-Corpus Back-Search Rescue
    print("\n4. Applying Occurrence Sampling Sieve & Rescuing Occurrences...")
    ...
    print(f"  Cards rescued from corpus back-search: {rescued_from_corpus_count}")
    ...
    print("\n6. Executing ClosedMenu WSD (es-lyrics-v18-1)...")
    import spacy
    ...
    for item in raw_index:
        ...
        if len(senses) == 1:
            ...
            for ex_idx, ex in enumerate(examples_to_assign):
                enriched_ex = {
                    **ex,
                    "assignment_method": "es-lyrics-v18-1",
                    "prompt_id": "lyrics-v18-declared-deterministic" if "overlay" in sense["source"] else "lyrics-v18-monosemous-deterministic",
                ...
                card_decisions.append({
                    ...
                    "provenance": {
                        "assignment_method": "es-lyrics-v18-1",
                        "prompt_id": "lyrics-v18-deterministic",
                        "run_ts": datetime.now(UTC).strftime("%Y-%m-%dT%H:%MZ"),
                    },
            ...
            for ex_idx, ex in enumerate(examples_to_assign):
                ...
                enriched_ex = {
                    **ex,
                    "assignment_method": "es-lyrics-v18-1",
                    "prompt_id": "lyrics-v18-polysemous-embedding",
                    "run_ts": datetime.now(UTC).strftime("%Y-%m-%dT%H:%MZ"),
                    "confidence": conf,
                    "band": band,
                }
        ...
        final_index.append({
            **item,
            ...
            "sense_prompt_ids": ["es-lyrics-v18-1"] * len(active_senses),
    ...
    print(f"  Dummy fallback lines used: {dummy_fallback_used_count} (down from v16's ~1,454 and v17's 306).")
    ...
    final_evidence = {
        ...
        "selection_projection": "mwe_augmented",
        "source_kind": "lyrics_wsd_v18_pipeline",
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plant artist discographies on Lyrics WSD v18")
    parser.add_argument("--artist", type=str, default="all", help="Artist slug or 'all'")
    ...
    artists = ["rosalia", "young-miko", "bad-bunny"] if args.artist == "all" else [args.artist]

    unified_release_dir = workspace / "releases" / "lyrics" / "lyrics-all-artists-v18" / "app" / "Artists" / "es"

    for artist_slug in artists:
        src = source_base / artist_slug
        # 1. Output to per-artist release
        artist_out = workspace / "releases" / "lyrics" / f"lyrics-{artist_slug}-v18" / "app" / "Artists" / "es" / artist_slug
        plant_artist(artist_slug, src, artist_out, workspace, repo_root)

        # 2. Output to unified all-artists release
        unified_out = unified_release_dir / artist_slug
        plant_artist(artist_slug, src, unified_out, workspace, repo_root)

    # If spanish-test-playlist exists, copy it into unified release
    test_src = workspace / "releases" / "lyrics" / "lyrics-test-playlist-v16-20260925" / "app" / "Artists" / "es" / "spanish-test-playlist"
    if test_src.is_dir():
        import shutil
        test_out = unified_release_dir / "spanish-test-playlist"
        test_out.mkdir(parents=True, exist_ok=True)
        for f in test_src.glob("*.json"):
            shutil.copy2(f, test_out / f.name)
        print(f"\nCopied spanish-test-playlist into unified release: {test_out}")
