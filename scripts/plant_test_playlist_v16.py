        ...
        for line in handle:
            for w in target_words:
                if f'"{w}"' in line:
                    ...
                    word_val = (data.get("word") or "").strip().lower()
                    if word_val in target_words:
                        entries[word_val].append(data)
                    break
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plant test playlist on Lyrics WSD v16")
    parser.add_argument("--source", type=Path, default=SOURCE_PLAYLIST_PATH)
    ...
    print("================================================================================")
    print("Lyrics WSD v16 Plant Pipeline: spanish-test-playlist")
    ...
    overlay_provider = build_lyrics_overlay_provider(repo_root, "es", workspace=workspace, artist="bad-bunny")
    declared_registry = stack(repo_root, "es", workspace=workspace, artist="bad-bunny")
    ...
    kaikki_cache_path = workspace / "raw" / "cache" / "kaikki" / "spanish-test-playlist-kaikki.json"
    ...
    for card_id, orig_card in raw_master.items():
        ...
        # Priority B: Proper noun resolution via Wikipedia API
        if is_propn or orig_card.get("extra_category") == "proper_noun":
        ...
        # Priority C: Wiktionary senses from Kaikki
    ...
    delta_cache_path = workspace / "raw" / "cache" / "embeddings" / "spanish-test-playlist-delta.npz"
    ...
    final_evidence = {
        "artist_slug": "spanish-test-playlist",
